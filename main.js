...
const time = document.getElementsByClassName('time-value')[0]
let timeStart
...
function gameEnd() {
  const timeValue = new Date(Date.now() - timeStart)
  time.innerHTML = `${timeValue.getMinutes()} M ${timeValue.getSeconds()} c`
  score.innerHTML = `${currentWord} из ${data.length}`
...
// result
const buttonReplay = document.getElementsByClassName('menu_button')[1]
const score = document.getElementsByClassName('score-value')[0]

buttonReplay.addEventListener('click', gameInitialization)
